...
from src.utils.config_util import DetectBoxInfo, BLOCK_DETECTION_TIME
...
class EntranceBoxZone:
    """
    피더 입구 막힘 감지 박스 영역
    객체의 중앙점이 박스 안에 들어오면 AirKnife에 부는 형태로 카운팅
    """
    ...
    def update(self, obj: DetectedObject) -> bool:
        ...
        if inside and is_target:    # 감지 박스 안에 있고 감지할 대상이라면
            if obj.id not in self.tracked_objects:  # obj.id가 새롭게 들어온 경우(새로운 물체)
                ...
                # 이미 박스 안에 있는 물체가 일정 시간 이상 나가지 않고 머물러 있다면?
                # -> 뭔가 막혔다! (조건문을 어떤식으로 작성할건지)
                # -> airknife_callback() 호출 -> True 반환

                # 마지막으로 들어온 객체가 여전히 박스 안에 있는 경우
                if obj.id == self.last_obj_id:
                    # 막힘 감지 시간 이상 같은 객체가 있으면 막힘으로 간주
                    if current_time - self.last_in_time > self.block_detection_time:
