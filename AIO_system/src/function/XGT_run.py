...
class XGTTester:
    ...
    def connect(self):
        ...
        try:
            ...
            self.connected = True
            log(f"✅ PLC {self.ip}:{self.port}에 연결되었습니다.")
            return True
        except Exception as e:
            log(f"[ERROR] ❌ PLC 연결 실패: {str(e)}")
            self.connected = False
            return False
    ...
    def send_packet_to_plc(self, packet, description=""):
        ...
        try:
            if description:
                log(f"\n===== {description} =====")
            # log(f"전송 패킷 (hex): {packet.hex()}")
            self.sock.send(packet)
            response = self.sock.recv(1024)
            # log(f"응답 패킷 (hex): {response.hex()}")

            # 응답 상태 확인
            if len(response) >= 28:
                status = response[26:28]
                if status == b'\x00\x00':
                    # log("✅ 성공! 통신이 정상적으로 이루어졌습니다.")
                    return True, response
                else:
                    # log(f"[WARNING] ❌ 실패! 상태 코드: {status.hex()}")
                    return False, response
            else:
                log("[WARNING] ❌ 응답이 충분하지 않음")
                return False, response
        except Exception as e:
            log(f"[ERROR] ❌ 통신 오류: {str(e)}")
            self.connected = False
            return False, None
    ...
    def write_d_value(self, value):
        ...
        # log(f"D00000에 값 {value} 쓰기 시도 중...")
        success, response = self.send_packet_to_plc(packet, f"D00000에 값 {value} 쓰기")

        if response:
            log(f"응답 패킷: {response.hex()}")

        if success:
            log(f"D00000에 값 {value} 쓰기 성공!")
        else:
            log("D00000에 값 쓰기 실패!")
    ...
    def read_mx_bit(self, address):
        ...
        if success and len(response) >= 30:
            ...
            # log(f"%MX{address} 현재 값: {bit_value} ({'ON' if bit_value else 'OFF'})")
            return bit_value
        return None
    ...
    def write_d_and_set_m300(self, d_value):
        ...
        # log(f"1. D00000에 {d_value} 값을 쓰고")
        # log("2. M300 비트를 ON(1)으로 설정합니다.")
        # log("=================================================")

        # 1. M300 초기 상태 확인
        initial_m = self.read_mx_bit(300)
        # log(f'initial_m ;{initial_m}')

        # 2. D00000에 값 쓰기
        d_success = self.write_d_value(d_value)
        if not d_success:
            log("D00000 쓰기 실패! M300 비트 설정을 건너뜁니다.")
            return False
    ...
    def write_set_d_value(self, d_address=None, value=0):
        ...
        success, response = self.send_packet_to_plc(packet, f"D00000에 값 {value} 쓰기")
        if response:
            log(f"응답 패킷: {response}")

        if success:
            log(f"{d_address}에 값 {value} 쓰기 성공!")
        else:
            log(f"{d_address}에 값 쓰기 실패!")

        return success

    def _create_bit_packet(self, address: int, onoff: Optional[bool]) -> bytearray:
    ...
    def _create_status_packet(self):
        """헬스 체크용 패킷 생성"""
        packet = bytearray()
        packet.extend(b'LSIS-XGT')  # 고정
        packet.extend(b'\x00\x00')  # Reserved (무시)
    ...
    def read_bit_packet(self, address: int) -> Optional[int]:
        """해당 주소에 저장된 비트 값 읽기"""
        packet = self._create_bit_packet(address, None)
        ret, response = self.send_packet_to_plc(packet)
        if ret and len(response) >= 30:
            bit_value = response[29]
            return bit_value
        elif not response:
            log("[WARNING] PLC에서 연결을 종료했습니다.")
            self.disconnect()
        return None

    def write_bit_packet(self, address: int, onoff) -> bool:
        """해당 주소에 비트 값 쓰기"""
        packet = self._create_bit_packet(address, onoff)
        ret, response = self.send_packet_to_plc(packet)
        if not response:
            log("[WARNING] PLC에서 연결을 종료했습니다.")
            self.disconnect()
